...
try {
    ...
    Write-Host ''; Write-Status PASS 'ASSESSMENT 2 SETUP COMPLETE'; Write-Status INFO "Node.js: $((& $nodeCommand --version).Trim())"; Write-Status INFO "npm: $npmVersion"; Write-Status INFO 'Dependencies: restored from package-lock.json'; Write-Status INFO 'Tests and syntax checks: passed'; Write-Status INFO 'Run the game with: Terminal > Run Task > BFC: Run Tank Arena'; Write-Status INFO 'Or run: powershell -NoProfile -ExecutionPolicy Bypass -File .\Run-Assessment2.ps1 start'; Write-Status INFO 'Then open: http://localhost:3000'; Write-Status INFO 'The supplied Run/Test workflow resolves BFC Node directly; the optional user PATH entry is not required.'
}
